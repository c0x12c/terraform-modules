...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_cloudwatch_logging"></a> [cloudwatch\_logging](#module\_cloudwatch\_logging) | ../terraform-aws-eks-cloudwatch-logging-fargate | n/a |
| <a name="module_datadog_rbac"></a> [datadog\_rbac](#module\_datadog\_rbac) | ../terraform-aws-eks-datadog-rbac | n/a |
| <a name="module_efs"></a> [efs](#module\_efs) | ../terraform-aws-eks-efs | n/a |
| <a name="module_eks_managed_node_group"></a> [eks\_managed\_node\_group](#module\_eks\_managed\_node\_group) | ../terraform-aws-eks-managed-node-group | n/a |
| <a name="module_fargate_profile"></a> [fargate\_profile](#module\_fargate\_profile) | ../terraform-aws-eks-fargate-profile | n/a |
...
| <a name="input_addons_metrics_server_version"></a> [addons\_metrics\_server\_version](#input\_addons\_metrics\_server\_version) | metrics-server add-on version; null = AWS default/recommended. | `string` | `null` | no |
...
| <a name="input_enable_metrics_server"></a> [enable\_metrics\_server](#input\_enable\_metrics\_server) | Install the metrics-server EKS managed add-on (opt-in; required for HPA cpu/memory metrics). | `bool` | `false` | no |
...
| <a name="input_metrics_server"></a> [metrics\_server](#input\_metrics\_server) | Configuration for the metrics-server add-on | <pre>object({<br/>    node_selector = optional(map(string), {})<br/>    tolerations = optional(list(object({<br/>      key      = string<br/>      operator = string<br/>      value    = optional(string)<br/>      effect   = optional(string)<br/>    })), [])<br/>  })</pre> | <pre>{<br/>  "node_selector": {},<br/>  "tolerations": []<br/>}</pre> | no |
| <a name="input_name"></a> [name](#input\_name) | The prefix name that will be used in cluster and components's name. | `string` | n/a | yes |
| <a name="input_node_group_version"></a> [node\_group\_version](#input\_node\_group\_version) | Kubernetes version for the managed node group. Null tracks cluster\_version. | `string` | `null` | no |
