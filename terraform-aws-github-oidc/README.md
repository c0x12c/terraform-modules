...
Repositories created, renamed, or transferred after 2026-07-15 send a
[subject claim with immutable org and repository IDs](https://github.blog/changelog/2026-04-23-immutable-subject-claims-for-github-actions-oidc-tokens/):

```text
repo:my-org/my-repo:ref:refs/heads/main            # legacy
repo:my-org@240785894/my-repo@1323742456:ref:...   # immutable
```

A trust policy that only allows `repo:my-org/*` rejects the new format. On top of the `sub` values you
pass in `conditions`, the module always allows the immutable form of `repository_path` — so
`repository_path = "my-org/my-repo"` also allows `repo:my-org@*/my-repo@*`, and `"my-org/*"` allows
`repo:my-org@*/*@*`. Both formats work; nothing outside the org is accepted.

Note this ignores any branch or environment scoping in your `conditions` values. For a role that must
stay pinned to a branch, add the immutable subject yourself with the exact IDs
(`repo:my-org@240785894/my-repo@1323742456:ref:refs/heads/main`) and keep `repository_path` on that repo.
