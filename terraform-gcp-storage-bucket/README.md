...
| <a name="input_lifecycle_rules"></a> [lifecycle\_rules](#input\_lifecycle\_rules) | List of lifecycle rules for the bucket | <pre>list(object({<br/>    age                        = number // days<br/>    type                       = string // action type<br/>    days_since_noncurrent_time = number // days<br/>  }))</pre> | `[]` | no |
| <a name="input_location"></a> [location](#input\_location) | The Google Cloud Storage Service location. | `string` | `"US"` | no |
| <a name="input_logging"></a> [logging](#input\_logging) | Define this block whether to enable logging | <pre>object({<br/>    enabled            = bool<br/>    destination_bucket = string<br/>  })</pre> | `null` | no |
