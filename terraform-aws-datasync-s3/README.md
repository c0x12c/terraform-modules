...
| <a name="input_create_cloudwatch_log_resource_policy"></a> [create\_cloudwatch\_log\_resource\_policy](#input\_create\_cloudwatch\_log\_resource\_policy) | Whether to create the log resource policy letting DataSync write to CloudWatch Logs. The policy is account-wide, and CloudWatch Logs caps resource policies at 10 per region, so set false on additional instances. | `bool` | `true` | no |
