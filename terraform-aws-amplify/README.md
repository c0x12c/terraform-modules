...
| <a name="input_enable_build_webhook"></a> [enable\_build\_webhook](#input\_enable\_build\_webhook) | Create the incoming build webhook used to kick the first build. Set false on aws provider versions that cannot read aws\_amplify\_webhook back. | `bool` | `true` | no |
...
| <a name="input_github_token"></a> [github\_token](#input\_github\_token) | GitHub token for authorizing with GitHub, passed to the app's access\_token. The Amplify API caps this at 255 characters; use github\_oauth\_token for anything longer. | `string` | `null` | no |
| <a name="input_github_oauth_token"></a> [github\_oauth\_token](#input\_github\_oauth\_token) | GitHub token passed to the app's oauth\_token instead of access\_token, which allows up to 1000 characters. Use for GitHub App installation tokens (ghs\_), which exceed the 255-character access\_token limit. Takes precedence over github\_token. | `string` | `null` | no |
