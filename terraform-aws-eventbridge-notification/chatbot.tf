...
resource "aws_chatbot_teams_channel_configuration" "this" {
  for_each = var.teams_channels

  configuration_name = coalesce(each.value.configuration_name, "${var.name}-${each.key}-teams")
  iam_role_arn       = coalesce(each.value.iam_role_arn, local.chatbot_role)
  ...
  tags = merge(var.tags, {
    Name = coalesce(each.value.configuration_name, "${var.name}-${each.key}-teams")
  })

  lifecycle {
    precondition {
      condition     = each.value.iam_role_arn != null || local.chatbot_role != null
      error_message = "Teams channel \"${each.key}\" has no IAM role: set create_iam_role, iam_role_arn, or the channel's own iam_role_arn."
    }
  }
}
