...
|------|-------------|------|---------|:--------:|
| <a name="input_groups"></a> [groups](#input\_groups) | A map of IAM SSO groups, their users, AWS accounts, and permission sets. | <pre>map(object({<br/>    # Map of AWS account IDs to permission set ARNs<br/>    aws_accounts = optional(map(map(string)))<br/>    users = optional(<br/>      map(<br/>        object({<br/>          first_name = string<br/>          last_name  = string<br/>          user_name  = string<br/>        })<br/>      )<br/>    )<br/>  }))</pre> | `{}` | no |
