...
locals {
  default_rds_monitors = {
    ...
    rds_query_p95 = {
      ...
      query_template = "percentile($${timeframe}):p95:trace.postgresql.query{env:${var.environment}, service:${var.service_name}} by {resource_name} > $${threshold_critical}"
      query_args = {
        timeframe = "last_5m"
      }
    ...
    rds_query_hits = {
      ...
      query_template = "sum($${timeframe}):sum:trace.postgresql.query.hits{env:${var.environment}, service:${var.service_name}}.as_count() > $${threshold_critical}"
      query_args = {
        timeframe = "last_5m"
      }
    ...
    rds_query_errors = {
      ...
      query_template = "sum($${timeframe}):sum:trace.postgresql.query.errors{env:${var.environment}, service:${var.service_name}}.as_count() > $${threshold_critical}"
      query_args = {
        timeframe = "last_5m"
      }
